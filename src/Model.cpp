...
Model::Model(MbRandom *rp, Alignment *ap, string ts, double pm, double ra, double rb, 
             ...
             int specPr, int psiPr, double bPrRate, double dPrRate) {
...
//        initOT = 150.0;
	...
	for (int i=0; i<2; i++){ 
		...
		parms[i].push_back( new Speciation(ranPtr, this, bdr, bda, bds, initRootH, rho, fbdPar, fxPsi, psi, specPr, psiPr, bPrRate, dPrRate) );												// hyper prior on diversification for cBDP speciation
...
Model::Model(MbRandom *rp, std::string clfn, int nodpr, double rh, bool rnp, int bdp, bool fixFRG, bool lnSurf, bool fxPsi, double psi, bool compS, int specPr, int psiPr, double bPrRate, double dPrRate){
    ...
    if(treeTimePrior == 9){
        ...
        Speciation *sp = new Speciation(ranPtr, this, -1.0, -1.0, -1.0, 100.0, rho, bdp, fxPsi, psi, specPr, psiPr, bPrRate, dPrRate);
        for (int i=0; i<2; i++){
    ...
    if(treeTimePrior == 10){
        ...
        Speciation *sp = new Speciation(ranPtr, this, -1.0, -1.0, -1.0, 100.0, rho, fbdPar, fxPsi, psi, specPr, psiPr, bPrRate, dPrRate);
